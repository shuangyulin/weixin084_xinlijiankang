	...
	export default {
		...
		async onLoad(options) {
			...
			if (options.id) {
				...
				// 获取信息
				res = await this.$api.info(`discussxinlichanpin`, this.ruleForm.id);
		...
		methods: {
			...
			async onSubmitTap() {
				...
				if(this.ruleForm.id){
					await this.$api.update(`discussxinlichanpin`, this.ruleForm);
				}else{
					await this.$api.add(`discussxinlichanpin`, this.ruleForm);
				}
				uni.setStorageSync('discussxinlichanpinCleanType',true);

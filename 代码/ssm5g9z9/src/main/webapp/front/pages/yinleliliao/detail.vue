	...
	<mescroll-uni @init="mescrollInit" :up="upOption" :down="downOption" @down="downCallback" @up="upCallback" :bottom="100">
		<view class="container">
			...
			<view class="price detail-list-item" :style='{"boxShadow":"0 0 0px rgba(0,0,0,.3)","margin":"0 0 2rpx 0","borderColor":"rgba(171, 174, 176, 1)","borderRadius":"0","borderWidth":"2rpx","borderStyle":"none none solid none","height":"68rpx"}'>
				...
				<view class="detail-list-item" :style="{height:'auto',lineHeight:1,fontSize:0,padding: '20rpx 24rpx',margin:'0 0 2rpx 0',boxShadow:'0 0 0px rgba(0,0,0,.3)'}">
            ...
            <view class="time-content" :style='{"boxShadow":"0 0 0px rgba(0,0,0,.3)","margin":"0 0 2rpx 0","borderColor":"#fff","backgroundColor":"#fff","borderRadius":"0","borderWidth":"0","borderStyle":"solid","height":"auto"}' style="height:auto !important;">
				...
				<view class="content" :style='{"color":"#333","fontSize":"28rpx"}'>
					<rich-text style="font-size: 33upx;line-height: 50upx;letter-spacing: 5upx;" :nodes="detail.xiangxishuoming"></rich-text>
				</view>
			</view>
...
<script>
	export default {
		...
		async onShow(options) {
			...
			this.getStoreup();
			let cleanType = uni.getStorageSync('discussyinleliliaoCleanType')
			if(cleanType){
				uni.removeStorageSync('discussyinleliliaoCleanType')
				this.mescroll.num = 1
		...
		methods: {
			// 支付
			onPayTap(){
				uni.setStorageSync('paytable','yinleliliao');
			...
			onAcrossTap(tableName){
     			uni.setStorageSync('crossTable','yinleliliao');
				uni.setStorageSync(`crossObj`, this.detail);
				this.$utils.jump(`../${tableName}/add-or-update?cross=true`);
			},
			// 获取详情
			async init(){
				let res = await this.$api.info('yinleliliao', this.id);
				this.detail = res.data;
				// 轮播图片
				this.swiperList = this.detail.fengmian ? this.detail.fengmian.split(",") : [];
				//修改富文本的图片样式
				this.detail.xiangxishuoming = this.detail.xiangxishuoming.replace(/img src/gi,"img style=\"width:100%;\" src");
			},
			...
			async upCallback(mescroll) {
                let res = await this.$api.list('discussyinleliliao', {
					page: mescroll.num,
					limit: mescroll.size,
					refid: this.id
				});
			...
			// 添加评论
			onCommentTap() {
				this.$utils.jump(`../discussyinleliliao/add-or-update?refid=${this.id}`)
			},
			// 点赞
			async zan() {
				let _this = this;
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							...
							await _this.$api.update('yinleliliao', _this.detail);
			...
			async onFinishTap() {
				...
				await this.$api.update('yinleliliao', this.detail);
				this.$utils.msg('审核成功');
